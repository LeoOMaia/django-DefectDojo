...
class ListFindings(View, BaseListFindings):
    ...
    def filter_findings_by_risk(self, findings, user_assessments, model_inferences, risk_filter):
        return findings.filter(
                id__in=[
                    fid for fid in findings.values_list("id", flat=True)
                    if user_assessments.get(str(fid), model_inferences.get(str(fid), "NA")) in risk_filter
                ],
            )

    def sort_findings_by_risk(self, findings, user_assessments, model_inferences, *, reverse=False):
        return sorted(
            findings,
    ...
    def get(self, request: HttpRequest, product_id: int | None = None, engagement_id: int | None = None, test_id: int | None = None):
        ...
        user_assessments = get_user_assessments(request.user.id)
        model_inferences = get_model_inferences(request.user.id)
        filtered_findings = self.get_fully_filtered_findings(request)
        ordered_findings = filtered_findings.qs
        risk_filter = request.GET.getlist("risk")
        if risk_filter:
            ordered_findings = self.filter_findings_by_risk(ordered_findings, user_assessments, model_inferences, risk_filter)
        ...
        if order_field:
            ...
            if order_field == "risk":
                ordered_findings = self.sort_findings_by_risk(
                    ordered_findings,
                    user_assessments,
                    model_inferences,
        ...
        paged_findings = get_page_items(request, ordered_findings, 25)
...
class ViewFinding(View):
    ...
    def get(self, request: HttpRequest, finding_id: int):
        ...
        # get the user assessments and model inferences
        user_assessments = get_user_assessments(request.user.id)
        model_inferences = get_model_inferences(request.user.id)
        context["user_assessments"] = user_assessments
        context["model_inferences"] = model_inferences
        # Render the form
        return render(request, self.get_template(), context)
...
def calculate_possible_related_actions_for_similar_finding(
    ...
    elif similar_finding.duplicate_finding:
    ...
    elif finding.duplicate or finding.original_finding.all():
        actions.extend((
            {
                "action": "mark_finding_duplicate",
                "reason": "Will mark this finding as duplicate of the root finding in this cluster",

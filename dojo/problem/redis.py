...
def add_finding_to_redis(finding):
    if finding.vuln_id_from_tool and finding.severity != 'Info':
        ...
        if redis_client.exists('problems', 'id_to_problem'):
            ...
            problem_id = script_to_problem_mapping.get(script_id)

            # Encode problem_id and script_id to base64 to avoid problems with special characters
            script_id = base64.b64encode(script_id.encode()).decode()
            problem_id = base64.b64encode(problem_id.encode()).decode() if problem_id else script_id
...
def dict_problems_findings():
    ...
    # Create a new problems and id_to_problem dict
    script_to_problem_mapping = problems_help.load_json()
    problems = {}
    id_to_problem = {}
    ...
    id_to_problem_data = {key: value for key, value in id_to_problem.items()}
...
def find_or_create_problem(finding, script_to_problem_mapping, problems, id_to_problem):
    ...
    if problem_id in problems:
        ...
        # Create a new problem with problem_id
        problems[problem_id] = Problem(finding.title, problem_id, finding.severity)
        problems[problem_id].finding_ids.add(finding.id)
        problems[problem_id].script_ids.add(finding.vuln_id_from_tool)
        id_to_problem[finding.id] = problem_id